...
function buildGoogleUrl(appName: string) {
  ...
  return `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri as string)}&response_type=code&scope=openid%20email%20profile&prompt=select_account&state=${encodeURIComponent(state)}`
}

export function signInWithGoogle(appName = 'Modulate') {

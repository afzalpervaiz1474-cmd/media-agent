...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const { data: item, error } = await supabase.from('media_assets').select('*').eq('id', id).eq('user_id', user.id).maybeSingle();
      if (error) throw error;
      if (!item) return res.status(404).json({ error: 'Not found' });

      const signedUrl = await getSignedUrl(item.storage_path);
      return res.status(200).json({ item: { ...item, public_url: signedUrl || item.public_url } });
    }
